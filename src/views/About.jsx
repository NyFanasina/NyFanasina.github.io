...
import { GiBearFace, GiCoconuts, GiPalmTree } from "react-icons/gi";
    ...
    <div className="sm:py-20 py-10 text-center mt-5" id="about">
      ...
      <div className="grid grid-cols-3 min-[480px]:grid-cols-4 sm:grid-cols-5 md:grid-cols-6 lg:grid-cols-6 xl:grid-cols-9 gap-6 place-items-center mt-20">
        {skills.map(({ icon, label }, index) => (
          <div
            key={index}
            className="flex flex-col items-center justify-center gap-3 w-28 h-28 bg-white/10 backdrop-blur-sm border border-white/10 rounded-2xl text-white shadow-md transition-all duration-300 hover:scale-105 hover:text-cyan-400 hover:shadow-[0_0_15px_#06b6d4]"
          >
